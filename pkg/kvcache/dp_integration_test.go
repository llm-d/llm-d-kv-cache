...
func TestDPMsgpackEventParsing(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Note: vLLM 0.16.0 always sends 3-element array with nil dp_rank for non-DP.
		// A 2-element array (no dp_rank field) would fail msgpack array decoding.
	} {
		t.Run(tc.name, func(t *testing.T) {
			var batch []any
			if tc.dpRank != nil {
				batch = []any{1234567890.0, []any{blockStoredEvent}, *tc.dpRank}
			} else {
				batch = []any{1234567890.0, []any{blockStoredEvent}}
			}
