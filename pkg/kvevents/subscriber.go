...
func (s *subscriber) runSubscriber(ctx context.Context) {
	...
	for {
		...
		// Receive raw message from adapter
		// Decoding is done within the pool loop for efficiency
		msg, err := s.adapter.ReceiveMessage(ctx)
		if err != nil {
			if ctx.Err() != nil {
				// Context was canceled, exit gracefully
				return
			}
			debugLogger.Error(err, "Failed to receive message", "endpoint", s.endpoint)
			break // Exit on receive error to reconnect
		}

		debugLogger.V(logging.TRACE).Info("Received raw message",
			"topic", msg.Topic,
			"seq", msg.Sequence,
			"podID", msg.PodID,
			"modelName", msg.ModelName)

		// Push raw message to pool
		s.pool.AddTask(msg)
	}
}
