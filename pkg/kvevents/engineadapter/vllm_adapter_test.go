...
package engineadapter //nolint:testpackage // Tests access unexported functions parseVLLMMessage and decodeVLLMEvent
...
func TestDecodeVLLMEvent_BlockStored(t *testing.T) {
	...
	// ExtraKeys is not included here as it is only present in newer vLLM versions.
	vllmEvent := []any{
		"BlockStored",                   // Tag
...
func TestDecodeEventBatch_NestedArrayEvents(t *testing.T) {
	...
	// Build a BlockStored event as a nested msgpack array (exactly what vLLM sends)
	...
	// Encode the batch: [ts, [event1, event2, ...], dataParallelRank]
	batch := []any{
		1234567890.0,            // TS
		[]any{blockStoredEvent}, // Events: array of arrays
	...
	err = msgpack.Unmarshal(payload, &vllmBatch)
	require.NoError(t, err, "batch decode must not fail with nested array events")
	require.Len(t, vllmBatch.Events, 1)

	// Decode the individual event from the raw bytes captured by RawMessage
	event, err := adapter.decodeVLLMEvent([]byte(vllmBatch.Events[0]))
	require.NoError(t, err)
	require.NotNil(t, event)
