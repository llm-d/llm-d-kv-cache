...
def cleanup_files(model_name, tp_size, tp_rank, dtype, root_dir, block_hashes):
    ...
    base_path = StorageOffloadingHandlers.get_kv_cache_base_path(
        model_name, tp_size, tp_rank, dtype, root_dir)
...
def total_block_size_mb(num_layers, num_heads, block_size, head_size, dtype,
                        num_blocks):
...
def roundtrip_once(*, model_name: str, tp_size: int, tp_rank: int,
                   dtype: torch.dtype, root_dir: str, num_layers: int,
                   num_blocks: int, block_size: int, num_heads: int,
                   head_size: int, read_block_ids: list[int],
                   write_block_ids: list[int], group_size: int,
                   threads_per_gpu: int):
    ...
    base_path = cleanup_files(model_name, tp_size, tp_rank, dtype, root_dir,
                              block_hashes)
    ...
    for h in block_hashes:
        assert os.path.exists(
            StorageOffloadingHandlers.get_file_name(
                base_path, h)), "missing file after PUT"
    ...
    file_size_mb = os.path.getsize(
        StorageOffloadingHandlers.get_file_name(
            base_path, block_hashes[0])) / (1024 * 1024)
...
def test_fs_backend_roundtrip_param(group_size: int, start_idx: int):
    ...
    threads_per_gpu = 8
    roundtrip_once(model_name=model_name,
                   tp_size=tp_size,
                   tp_rank=tp_rank,
                   dtype=dtype,
                   root_dir=root_dir,
