...
bool StorageOffloadEngine::async_store_gpu_blocks(
  ...
  cudaEventRecord(event, at::cuda::getDefaultCUDAStream().stream());
  // Wrap default_done event in shared_ptr with custom deleter
  auto default_done = std::shared_ptr<CUevent_st>(event, [](CUevent_st* evt) {
  ...
  for (size_t i = 0; i < dst_files.size(); i++) {
    ...
    auto future = m_thread_pool.enqueue([this,
      ...
      cudaStreamWaitEvent(tls_stream.stream(), default_done.get(), 0);
      ...
      try {
        ...
        if (err != cudaSuccess) {
          ...
          if (!success)
            std::cerr << "[ERROR] Store failed during file write: " << dst_file
                      << "\n";
        }
      } catch (const std::exception& e) {
        std::cerr << "[ERROR] Store failed for " << dst_file << ": " << e.what()
                  << std::endl;
        success = false;
      } catch (...) {
