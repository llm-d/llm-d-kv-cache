...
def crawler_process(
    ...
    logger.info(
        f"Crawler P{process_num} started - hex %16 in [{min_mod}, {max_mod}] (hex: {hex_chars})"
    )
    logger.info(
    ...
    try:
        while not shutdown_event.is_set():
            ...
            for file_path in stream_cache_files(cache_path, hex_modulo_range):
                ...
                # Track file-to-file transition timing
                ...
                try:
                    ...
                    if time_since_access < access_time_threshold_seconds:
                        ...
                        if files_skipped % 10000 == 0:
                            logger.info(
                                f"Crawler P{process_num} skipped {files_skipped} files (last: accessed {time_ago_minutes:.2f} min ago)"
                            )
                        continue
...
def stream_cache_files(
    ...
    try:
        ...
        for model_dir in os.scandir(cache_path):
            ...
            try:
                # Check if {path} exists or if tp_{N} is directly under {model}
                # If folder starts with "tp_", then {path} is missing
                tp_dirs = []
                for item in os.scandir(model_dir.path):
                    if not item.is_dir():
                        continue

                    if item.name.startswith("tp_"):
                        # {path} is missing, this is tp_{N}
                        tp_dirs.append(item.path)
                    else:
                        # This might be {path}, scan for tp_{N} inside
                        try:
                            for tp_item in os.scandir(item.path):
                                if tp_item.is_dir() and tp_item.name.startswith("tp_"):
                                    tp_dirs.append(tp_item.path)
                ...
                for tp_path in tp_dirs:
                    try:
                        ...
                        for rank_dir in os.scandir(tp_path):
                            if not rank_dir.is_dir() or not rank_dir.name.startswith(
                                "rank_"
                            ):
                                continue
                            ...
                            try:
                                ...
                                for dtype_dir in os.scandir(rank_dir.path):
                                    if not dtype_dir.is_dir():
                                        continue

                                    # Scan hash1 folders under {X} (hex, any length)
                                    for hash1_dir in os.scandir(dtype_dir.path):
                                        ...
                                        # Scan hash2 folder and files
                                        try:
                                            for hash2_dir in os.scandir(hash1_dir.path):
                                                if hash2_dir.is_dir():
                                                    # Files are directly under hash2
...
def logger_process(
    ...
    logger.info(
        f"Logger P9 thresholds: cleanup={cleanup_threshold}%, target={target_threshold}%"
    )
    ...
    try:
        while not shutdown_event.is_set():
            ...
            if usage:
                ...
                logger.info(
                    f"CACHE_PERCENT_LOG:{current_time:.3f},{usage.usage_percent:.2f},statvfs"
                )
                logger.info(
                ...
                if usage.usage_percent >= cleanup_threshold and not deletion_active:
                    logger.warning(
                        f"Logger P9: Usage {usage.usage_percent:.2f}% >= {cleanup_threshold}% - Triggering deletion ON"
                    )
                    logger.info(
                    ...
                    logger.info(
                        f"Logger P9: Usage {usage.usage_percent:.2f}% <= {target_threshold}% - Triggering deletion OFF"
                    )
                    logger.info(
                ...
                try:
                    ...
                    try:
                        ...
                        if current_batch:
                            ...
                            logger.info(
                                f"Deleted final batch: {deleted} files, {freed / (1024**3):.2f}GB freed"
                            )
                            current_batch = []
                            result_queue.put(
                                ("progress", total_files_deleted, total_bytes_freed),
...
class PVCEvictor:
    ...
    def __init__(self, config: Config):
        ...
        )  # Logger controls Deleter, Crawlers check this
        ...
        self.logger.info(
            f"  Logger Process: P{logger_process_num} (monitoring every {config.logger_interval}s)"
        )
        self.logger.info(
    ...
    def run(self):
        ...
        for i in range(self.config.num_crawler_processes):
            ...
            if min_mod == max_mod:
                ...
                hex_chars = f"'{format(min_mod, 'x')}'-'{format(max_mod, 'x')}'"
            self.logger.info(
                f"Started crawler P{i+1} (hex %16 in [{min_mod}, {max_mod}], hex: {hex_chars})"
            )
            ...
            while self.running:
                try:
                    ...
                    if not logger_process_obj.is_alive():
                        self.logger.error(
                            f"Logger P{logger_process_num} died, restarting..."
                        )
                        logger_process_obj = multiprocessing.Process(
                            target=logger_process,
                            args=(
