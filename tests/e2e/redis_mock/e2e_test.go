...
func (s *KVCacheSuite) TestCacheHitWithLocalTokenizer() {
	...
	tokens, offsets, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: prompt, AddSpecialTokens: true})
	s.Require().NoError(err)
	...
	tokens2, _, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: prompt, AddSpecialTokens: true})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestLocalTokenizerChatTemplateE2E() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			tokens, offsets, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: renderedPrompt, AddSpecialTokens: true})
			...
			tokens2, _, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: renderedPrompt2, AddSpecialTokens: true})
			s.Require().NoError(err)
...
func (s *KVCacheSuite) TestLocalTokenizerChatTemplateMultiTurnE2E() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			shortTokens, _, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: shortPrompt, AddSpecialTokens: true})
			s.Require().NoError(err)
			...
			extendedTokens, _, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: extendedPrompt, AddSpecialTokens: true})
			s.Require().NoError(err)
...
func (s *KVCacheSuite) TestLocalVsHFChatTemplateConsistency() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			localTokens, _, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: localRendered, AddSpecialTokens: true})
			s.Require().NoError(err)
			...
			// Tokenize again
			localTokens2, _, err := localTokenizer.Encode(&preprocessing.EncodeRequest{Text: localRendered2, AddSpecialTokens: true})
			s.Require().NoError(err)
