...
### Using the UDS tokenizer sidecar

By default the `online` example uses the embedded tokenizer. To use the UDS tokenizer sidecar instead, set `TOKENIZER_ENDPOINT` to the socket path:
...
```bash
TOKENIZER_ENDPOINT=/tmp/tokenizer/tokenizer-uds.socket \
go run examples/kv_events/online/main.go
```

For TCP mode, set `TOKENIZER_ENDPOINT` to a `host:port` address instead.

> **Note:** UDS sockets are not accessible across Docker network boundaries. If the `online` example runs inside a container, use TCP mode (`host:port`) instead of a socket path.
