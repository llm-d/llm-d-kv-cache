...
# Wait up to 60s for a pattern to appear in a log file; returns 1 on timeout.
wait_for_log() {
...
timeout 60s make run-example-only offline >offline.log 2>&1 &
pid=$!
...
timeout 60s make run-example-only online >online.log 2>&1 &
pid=$!
