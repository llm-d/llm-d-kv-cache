...
- **Tokenization** — HuggingFace's Rust tokenizers via Go bindings.
- **[pebbe/zmq4](https://github.com/pebbe/zmq4)** — Go bindings for ZeroMQ. Used for the event processing pool. Requires `libzmq` installed on the system.
- **Python** — required only for a CGO binding of `chat_completions_template` (used for vLLM chat-template rendering).
